...
Die Desktop-Oberfläche ist die **EINZIGE** Schnittstelle zwischen GF und dem KI-System. Kein direkter Ordnerzugriff. Kein direkter Agenten-Kontakt außerhalb der GUI. Alles läuft über die GUI — einschließlich direkter Kommunikation: GF kann über die GUI gezielt Fragen an einen zuständigen Agenten stellen ("Was ist hier los?"), der Agent antwortet im selben Kanal.
...
### 13.2 Zugriff & Fünf Module

GUI ist erreichbar via PC / Arbeitsplatz **und** Smartphone / mobil — dieselbe Oberfläche, identischer Funktionsumfang. So kann der GF auch unterwegs Freigaben erteilen, Alarme sehen und mit Agenten kommunizieren.
Technische Umsetzung und Sicherheitsprotokoll für den mobilen Zugriff werden separat definiert (siehe 13.8).
...
### 13.7a Auftragsdetailansicht

Jeder Auftrag im Modul AUFTRÄGE ist anklickbar und öffnet eine Detailansicht
mit allen Einzelschritten, dem jeweils zuständigen Agenten und dem Status
je Schritt:
...
Aus dieser Ansicht heraus kann der GF direkt eine Frage an den zuständigen
Agenten des jeweiligen Schritts stellen (siehe 13.1).

### 13.7b Zeitfenster-Audit & Re-Trigger

Bei einem erkannten Fehler kann der GF nicht nur den einzelnen Vorfall prüfen,
sondern gezielt ein Zeitfenster um den Fehlerzeitpunkt definieren, um auch
unentdeckte Begleitfehler im selben Zeitraum aufzudecken.

```
GF setzt Zeitfenster manuell via GUI: z.B. 12:08–12:10
  → 11SC-02 prüft ALLE Aktionen in diesem Fenster
    (welche Agenten aktiv? welche Schritte? welche Daten bewegt?)
  → Bericht → Hermes + GF via GUI
  → Hermes identifiziert Unterbrechungspunkt
  → Hermes empfiehlt Re-Trigger ab exaktem Schritt
  → GF bestätigt via GUI → Prozess startet ab diesem Schritt neu
    (nicht von vorne)
```

**Prozess:** `P_007_Zeitfenster_Audit`
**Prozess:** `P_008_Re_Trigger_Auftrag`

### 13.8 GUI-Sicherheit

- **Login:** Nur GF — kein weiterer Benutzer
- **Authentifizierung:** Hardware-Token (YubiKey) + PIN am Arbeitsplatz; für mobilen Zugriff Authenticator-App + PIN (Details siehe `P_006_Zugriff_GUI`, wird separat definiert)
- **Session-Timeout:** 15 Minuten Inaktivität
- **IP-Bindung:** Für den festen Arbeitsplatz-Zugriff gilt IP-Bindung auf die definierte Workstation. Mobiler Zugriff ist zusätzlich erlaubt und läuft über ein separates Sicherheitsprotokoll (VPN-Pflicht o.ä.), das technisch noch festgelegt wird.
...
| A7 | Kommunikationsausfall | A02_Marketing + A03_Vertrieb gleichzeitig nicht erreichbar | 🔴 HIGH |
...
### A7 — Ausfall A02 + A03 (gleichzeitig)

```
11SC-01 erkennt Ausfall beider Abteilungen
  → Sofortalarm an Hermes + Super-Agent + GF via GUI

Hermes:
  → Stopp aller ausgehenden Kommunikationsaufträge
  → Alle offenen Aufträge → Status 04_Wartend
  → Prüft ob andere Abteilung temporär übernehmen kann

11SC:
  → Ursachenanalyse → Bericht an GF

GF entscheidet via GUI (PC oder mobil):
  → Temporäre manuelle Übernahme
  → Oder Reaktivierung nach 11SC-Bericht
  → Reaktivierung: nur mit expliziter GF-Freigabe
```

**Prozess:** `P_005_Kommunikationsausfall_A02_A03`
...
### 19.2 Backup-Typen & Retention

**Prinzip: Inkrementelles Backup**

```
Basis:   Vollbackup vom Vortag
Täglich: Nur neue Dateien des Tages werden gesichert
         → 01_INPUT  (neue Eingänge)
         → 03_OUTPUT (neue Ergebnisse)

Wiederherstellung: Vollbackup Vortag + gewünschter Tages-Increment
```
...
**Tresor-Backup:** Wird separat definiert (eigener Prozess, da K4-Daten besonders sensibel sind).

### 19.3 Wiederherstellungsprozess

**Verantwortung:** 11SC-03_FailoverManagement

```
GF initiiert via GUI
  → 11SC-03 identifiziert Vollbackup + benötigte Increments
  → 11SC-04 Hash-Validierung des Backup-Pakets
  → Hash OK → Wiederherstellung startet
  → Hash FAIL → Alarm an GF, nächstälterer Backup wird geprüft
  → Nach Wiederherstellung → 11SC-01 System-Check
  → GF-Freigabe für Wiederaufnahme Betrieb

RTO-Ziel (Recovery Time Objective): < 2 Stunden
RPO-Ziel (Recovery Point Objective): max. 1 Stunde Datenverlust
```

**Prozess:** `P_010_Backup_Wiederherstellung`
...
| Snapshot | SS_YYYYMMDD_v17.0.zip | SS_20260619_v17.0.zip |
...
- **Minor** (v17.0 → v17.1): Korrekturen, Optimierungen, Klarstellungen
- **Major** (v17.x → v18.0): Strukturelle Änderungen, neue Abteilungen, neue Architektur
...
**Prinzip:** Sonntag = Betriebsstopp. Keine Auftragsbearbeitung, keine Freigaben, keine Agenten-Aktivität außer Fortbildung und Systemcheck.

**Eingang:** Quarantäne läuft normal durch (alle drei Filterstufen aus Kap. 3) — Weiterverarbeitung startet Montag.

**Zeitfenster:** Sonntag 06:00–12:00 Uhr

```
06:00  11SC — Systemcheck
         → Testdurchlauf Gesamtsystem
         → Kontrolle ob alle Probleme der Vorwoche geschlossen wurden
         → Bericht → GF via GUI

07:00  Fachabteilungen A02–A13
         → Fortbildung im eigenen Fachgebiet
         → Ergebnis → 06_WISSEN/[Bereich]

09:00  A12_ZF + Super-Agent
         → Marktbeobachtung, Trends, KI-Entwicklungen

10:00  Hermes
         → Workflow-Bibliothek auf Aktualität prüfen
         → Offene Punkte der Vorwoche kontrollieren

11:00  Super-Agent
         → Gesamtbericht + Empfehlungen → GF

11:30  Bericht → GUI PROTOKOLL
         → GF sieht Sonntagsbericht montags beim Login

12:00  Abgeschlossen → Betrieb startet Montag
```

**Archiv:** `06_WISSEN/00_Sonntagsberichte/SB_[YYYY-MM-DD].md`
**Prozess:** `P_011_Sonntagsregel`
...
| `v[X]` | Versionsnummer | v17.0, v1, v2 |
| `[GROSS]` | Ordnernamen Wurzelebene | 01_INPUT, 04_TRESOR |
| `[klein]` | Dateinamen unterhalb Ordnerebene | regeln_v1.md, wissensbasis_v1.md |
...
**Auslöser:** Auftrag abgeschlossen, Output versendet (Status 04_Versendet).

| Quelle | Gewichtung | Erhebung |
|--------|-----------|---------|
| Kunde | 50% | Automatische Feedback-Anfrage nach Versand |
| GF | 30% | Bewertung bei Freigabe in GUI |
...
```
Score ≥ 4.0
  → Workflow in Bibliothek als "bewährt" markiert
  → Super-Agent meldet GF: "Checkpoint X reif für Autonomie"
  → GF entscheidet aktiv: Hermes übernimmt diesen Checkpoint oder bleibt assistiert

Score 3.0–3.9
  → Hermes analysiert → Verbesserungsvorschlag an GF

Score < 3.0
  → Prozess_Fehler_[OT-ID].md → A05-QM
  → A13_Personal leitet Ursachenanalyse ein (siehe 15a.8)
  → GF-Alarm
```

**Prozess:** `P_012_Feedback_Bewertung`

---

## 27. PROZESS-REGISTER

Alle Prozessdetails leben ausschließlich in den jeweiligen Prozessdokumenten. Die Systemdokumentation verweist nur auf die Prozess-ID. Änderungen am Prozess erfolgen nur im Prozessdokument selbst — die Systemdokumentation muss dafür nicht angepasst werden.
...
*System-Version: v17.0 (2026-06-19) | GUI-Version: GUI_v2.0 | Erstellt: 19.06.2026*
