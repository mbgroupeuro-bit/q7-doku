# KI-UNTERNEHMEN (Q7) — VOLLSTÄNDIGE SYSTEMDOKUMENTATION v19.0
**Erstellt:** 19.06.2026 | **Aktualisiert:** 19.06.2026 | **System-Version:** v19.0 (2026-06-19) | **GUI-Version:** GUI_v2.0
...
| **v19.0** | **19.06.2026** | **MAJOR: Neuer Kunden-Analyse & Transformationsprozess P_016 mit 11 Unterprozessen (P_016.1–P_016.11). Neuer Ordner `05_UNTERNEHMEN/09_Kunden` für Kunden-Stammdaten & Analyseergebnisse. Neuer Tresor-Bereich `04_TRESOR/K4_Personen/KD_[Nr]` für personenbezogene Kundendaten. Kap. 27 Prozess-Register erweitert. Kap. 32 Zusammenfassung v19.0 neu.** |
...
🧠 SUPER-AGENT (A00)
   ...
   ↓
🛡️  A14_SICHERHEIT (NEU v20.0)
   (Eigenständig, unabhängig, direkte GF-Linie — exklusiver Erstzugriff
   auf 01_INPUT/02_Eingang NACH Quarantäne, trennt personenbezogene
   Daten (PII) BEVOR Hermes zugreift, siehe Kap. 6a)
   ↓
...
- **A14_Sicherheit (NEU v20.0)** = eigenständig, unabhängig — keine Fachabteilung kann A14-Prüfung anweisen, beschleunigen oder umgehen (vergleichbar mit der Unabhängigkeit von A05_QM). Exklusiver Erstzugriff auf 01_INPUT/02_Eingang nach Quarantäne, BEVOR Hermes zugreift. Siehe Kap. 6a.
- **Hermes** = Abteilungsleiter Planung & BackOffice, koordiniert alle Aufträge, erstellt Workflows — erhält Kunden-Input erst NACH A14-Freigabe
...
### Die fünf Ebenen im Vergleich

| Ebene | Wer | Funktion |
|-------|-----|----------|
| Kontrolle | GF (Mensch) | Entscheidet, freigibt, stoppt |
| Beobachtung | Super-Agent | Überwacht, lernt, schlägt vor |
| **Datenschutz (NEU v20.0)** | **A14_Sicherheit** | **Trennt PII von Sachdaten, exklusiver Erstzugriff auf Eingang** |
| Koordination | Hermes-Agent | Plant, verteilt, prüft |
| Ausführung | Fachabteilungen A02–A14 | Spezialisierte Arbeit |
...
**NEU v19.0:** Ordner `09_Kunden` ergänzt (siehe Kap. 32 & P_016).
...
- **04_TRESOR/K4_Personen/KD_[Nr]_[Name]/:** Kontaktdaten, Zugangsdaten → personenbezogen, verschlüsselt, nur via DRI
- **05_UNTERNEHMEN/09_Kunden/KD_[Nr]_[Name]/:** Stammdaten, Brand Assets, Analysen → betriebliches Wissen, intern zugänglich

---

## 6a. ABTEILUNG A14_SICHERHEIT (NEU v20.0)

### Hintergrund

Im Praxisbetrieb (erster Realkunde, P_016) zeigte sich eine strukturelle Lücke: Niemand im System war explizit dafür zuständig, personenbezogene Daten (PII) aus frischem Kunden-Input herauszufiltern, BEVOR Hermes (A01) oder andere Agenten Zugriff erhalten. A01-Guardrail G6 verbietet Hermes zwar den direkten TRESOR-Zugriff, aber es fehlte die vorgelagerte Trenninstanz.

### Einordnung

A14_Sicherheit ist eine eigenständige, unabhängige Abteilung — gleichrangig zu A01–A13, mit direkter GF-Linie (vergleichbar mit der Unabhängigkeit von A05_QM). Sie ist bewusst **nicht** als Sub-Agent unter Hermes oder Systemcontrolling eingeordnet, da beide Optionen einen Kontrollwiderspruch erzeugen würden (Hermes hätte indirekt Weisungsgewalt über die Stelle, die ihn selbst einschränkt; Systemcontrolling ist eine beobachtende, keine operative Funktion).

### Kernablauf

```
01_INPUT/01_Quarantäne → Filter 1-3 bestanden
  ...
  → A14 prüft auf personenbezogene Daten (PII)
      JA → PII extrahieren → 04_TRESOR/K4_Personen/KD_[Nr]
           Bereinigte Datei wird ZURÜCK nach 01_INPUT/02_Eingang
           verschoben (kein separater Zwischenordner)
      NEIN → Datei verbleibt, Vermerk "keine PII" im Protokoll
  → ERST JETZT erhält Hermes (A01) Zugriff → P_016.1 kann starten
```

### Verhältnis zu bestehenden Sicherheitsfunktionen

| Funktion | Zuständigkeit | Abgrenzung zu A14 |
|----------|---------------|---------------------|
| H-01 (Quarantäne, in A01) | Technische Sicherheit (Malware, Format, 3-Filter) | Prüft NICHT auf PII — A14 startet erst danach |
| 08F-02 (DRI-Gatekeeper, in A08) | Laufende TRESOR-Verarbeitung (z. B. Rechnungsstellung) | A14 ist nur für die Eingangs-Trennung zuständig, nicht für laufenden Betrieb |
| 11SC-02 (Systemcontrolling) | RBAC-Verstöße systemweit, reaktiv | A14 verhindert proaktiv an der Quelle, statt nur zu überwachen |

### Vier-Säulen-Grundlagen

Vollständige Regeln, Guardrails, Wissensbasis und Entscheidungsprinzipien liegen unter `02_KI-UNTERNEHMEN/A14_Sicherheit/02_Grundlagen/` (gleiches Format wie A01–A13, siehe Kap. 15.1).
...
├── 📁 A13_Personal
└── 📁 A14_Sicherheit         ← NEU v20.0: eigenständig, unabhängig (Kap. 6a)
```
...
| A14_Sicherheit | 14S | Sicherheit — Datenschutz & Zugriffsschutz (NEU v20.0, eigenständig/unabhängig) |
...
**Ergänzung v19.0 — P_016 Kunden-Analyse & Transformationsprozess (11 Unterprozesse):**
...
| **P_016** | **Kunden-Analyse & Transformationsprozess (Haupt-Prozess)** | **GF + Hermes + Fachabteilungen** |
| P_016.1 | Kunden-Stammdaten-Aufnahme | GF + A14_Sicherheit (PII-Vortrennung) + Hermes + 08F-02 |
| P_016.2 | Mystery-Shopping-Physisch | GF + Team (Menschen) + Hermes |
| P_016.3 | Digital-Audit | A02_Marketing + A12_Zukunftsforschung |
| P_016.4 | SWOT-Analyse | A02_Marketing + A05_QM |
| P_016.5 | Branchen-Benchmarks | A12_Zukunftsforschung + A08_Finanzen |
| P_016.6 | Gap-Analyse-IST-Zustand | Hermes + A05_QM |
| P_016.7 | Kundengespräch-Wunschermittlung | GF (persönlich) + Hermes |
| P_016.8 | Bedarfsanalyse-Intern | Hermes + A02 + A07 + A08 |
| P_016.9 | SOLL-Zustand-Roadmap | Hermes + alle relevanten Abteilungen |
| P_016.10 | Präsentation-PDF-Erstellung | A02_Marketing + A09_Dokumentation |
| P_016.11 | Kunden-Freigabe-Umsetzungsstart | GF (persönlich) + Hermes |
...
## 32. ZUSAMMENFASSUNG — WAS IST NEU IN v19.0

| Bereich | Kapitel | Kurzbeschreibung |
|---------|---------|-------------------|
| Kunden-Prozess | 27 / 32 | P_016 Kunden-Analyse & Transformationsprozess mit 11 Unterprozessen |
...
| Digital Audit | P_016.3 | Vollständige digitale Bestandsaufnahme durch Q7-Agenten |
| SWOT | P_016.4 | Strukturierte Stärken/Schwächen-Analyse inkl. Strategie-Ableitungen |
| Benchmarks | P_016.5 | Branchen-KPIs mit Vergleichstabelle (Branche / Wettbewerb / Kunde) |
| IST-Zustand | P_016.6 | Gap-Analyse & vollständiger IST-Gesamtbericht |
...
| SOLL-Zustand | P_016.9 | Transformations-Roadmap mit 3 Phasen & Meilensteinen |
| Deliverables | P_016.10 | Präsentation (Slides) + PDF-Report für Kunden |
| Umsetzungsstart | P_016.11 | Kunden-Freigabe + Start regulärer Q7-Auftrag |

### Kernprinzip v19.0

```
Q7 berät ehrlich — auch wenn der Kunde etwas nicht hören will.
Markenidentität ist keine Option — sie ist strategische Voraussetzung.
Ohne Fundament keine effektive Positionierung, Differenzierung oder Marketing.
```

### P_016 Qualitäts-Flag

```
🔴 PREMIUM — Kein Kompromiss bei Modellwahl
Bestes verfügbares Claude-Modell für alle P_016-Phasen.
P_016 ist das Aushängeschild von Q7.
```

---

*System-Version: v19.0 (2026-06-19) | GUI-Version: GUI_v2.0 | Aktualisiert: 19.06.2026*
