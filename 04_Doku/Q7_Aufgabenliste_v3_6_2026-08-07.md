...
Version: v3.6
Datum: 2026-08-05 (Nachtrag, Sitzung 2)
...
| Q7-A-051 | hoch | Routing-Architektur bestätigen (Root Cause 404) | q7-app.html ist Mockup ohne echte Routen; a_Q7-code braucht echtes Next.js App-Routing. Fehlende page.tsx für /kalender, /aufgaben vermutete 404-Ursache | **Teilerledigt 02.08.2026** — /aufgaben behoben (echte page.tsx mit Prisma-Anbindung, siehe Bereits erledigt). /kalender weiterhin 404, kein Datenmodell/Seite vorhanden — siehe Q7-A-107 |
...
| Q7-A-077 | mittel | Terminologie-Konflikt SYSTEM-PROMPT "Q7-System-Evolution-Agent" | Eingespielter Fremd-SYSTEM-PROMPT verwendet durchgängig "GF" statt "Admin" sowie abweichende Code-Formate (A05-QM, H-01, 11SC-01). Klären: fehlerhafte Altversion, bewusst getrennter Kontext, oder teilweise verwertbar? | **Erledigt 02.08.2026** — Admin-Bestätigung: "GF" = Altlast, "Admin" bleibt bindend. Rest des Prompts explizit akzeptiert ("ansonsten alles ok") — widerspricht der früheren Verwerfen-Empfehlung in Q7-A-078, siehe dort |
| Q7-A-078 | niedrig | 3-Stufen-Validierungsmodell aus Fremd-SYSTEM-PROMPT | Pflichtfelder/Format/KI-Plausibilität-Prüfkette mit 15-Min-SLA. Empfehlung: verwerfen, bestehender Freigabe-Flow (PROTOKOLL+AUFGABEN, v1.1) deckt das bereits ab | **Entschieden 02.08.2026 (Kurskorrektur)** — Admin akzeptiert Prompt inkl. 3-Stufen-Modell ("ansonsten alles ok") statt Verwerfen-Empfehlung zu folgen. Trennzeichen-System (`-`/`_`/`v[X]`), H-01, 11SC-01 damit grundsätzlich in Kraft — Verhältnis zu bestehenden Konventionen (PROTOKOLL+AUFGABEN, Agentencode-Format) inhaltlich noch nicht im Detail abgeglichen, nur Terminologie-Ebene bestätigt |
...
| Q7-A-087 | hoch | Neues Geschäfts-Gmail-Konto einrichten | Aktuelles Testkonto (`mbgroupeuro@gmail.com`) enthält Newsletter-/Werbe-Rauschen, wird fälschlich als Task erfasst. Schritte: Konto anlegen, 2FA aktivieren, neues App-Passwort, `.env` aktualisieren (`GMAIL_USER`, `GMAIL_APP_PASSWORD`), Testlauf wiederholen | **Erledigt 01.08.2026** — dediziertes Konto `abuu4666@gmail.com` eingerichtet und im Einsatz |
| Q7-A-088 | mittel | Automatisches wiederkehrendes Polling | Aktuell nur manueller Start über CMD — Windows Task Scheduler o.ä., Intervall festlegen | **Erledigt 01.08.2026** — Windows Task Scheduler (`Q7_Gmail_Polling`, alle 2 Minuten), siehe Q7-A-096 |
...
| Q7-A-091 | hoch | Kanal-Adapter → Hermes-Trigger Verdrahtung | Datenbank-Insert (poll-gmail.js) steht, automatischer Klassifikations-Aufruf (`rufeKIAn()`) direkt danach noch zu verdrahten — verbindet Q7-A-085 technisch mit dem realen Adapter | **Erledigt 01.08.2026** — `trigger-hermes.ts` gebaut, End-to-End-Test erfolgreich (Reklamation korrekt klassifiziert, Konfidenz 🟢, Ziel-Agent A05_QM). Duplikat von Q7-A-097 |
...
| Q7-A-095 | hoch | Gmail-Konto-Wechsel & Newsletter-Rauschen | Test-Konto erfasst aktuell jede ungelesene Mail als Task (Newsletter-Rauschen). Dediziertes Geschäfts-Gmail-Konto separat einrichten, 2FA + neues App-Passwort, `.env` aktualisieren, Testlauf wiederholen; danach Filter-Logik (Absender/Label statt gesamter Posteingang) | **Teilerledigt 01.08.2026** — Konto-Wechsel erledigt (siehe Q7-A-087). Filter-Logik (Absender/Label) weiterhin offen, siehe Q7-A-090 |
| Q7-A-096 | mittel | Automatisches Polling statt manuellem Start | `scripts/poll-gmail.js` aktuell nur manuell via `node -r dotenv/config` gestartet — wiederkehrendes Polling einrichten (z.B. Windows Task Scheduler) | **Erledigt 01.08.2026** — Task Scheduler `Q7_Gmail_Polling`, alle 2 Minuten. Hinweis aus Praxistest: Task darf NICHT mit erhöhten Rechten (`/rl highest`) laufen — lief scheinbar erfolgreich, schrieb aber nichts |
| Q7-A-097 | hoch | Verdrahtung Kanal-Adapter → Hermes-Trigger | DB-Insert-Teil fertig, automatischer Klassifikations-Aufruf (rufeKIAn() direkt nach Insert) noch nicht verdrahtet | **Erledigt 01.08.2026** — siehe Q7-A-091 (Duplikat), End-to-End bestätigt inkl. Weiterverarbeitung bis Eingang-UI (02.08.2026) |
...
| Eingang-Seite Prisma-Anbindung | 01.08. | `app/input/page.tsx` von Mock-Daten (`INPUT_FILES`) auf echte `aufgaben`-Tabelle umgestellt (Server Component, direkter Prisma-Zugriff) |
| MEINE AUFGABEN — Datenmodell + UI (Quellen: manuell, kanal_verlegt) | 01.08. | Neues Prisma-Modell `MeineAufgabe`, API-Routen (`/api/meine-aufgaben`, `/[id]`, `/aus-kanal`), Seite `/aufgaben` (behebt 404), Verlegen-Button auf Eingang-Seite. Quelle „Freigabe" bewusst noch nicht angebunden, siehe Q7-A-102 |
| Q7-Logo eingefügt | 02.08. | Finales Blütenblätter-Logo (`public/q7-logo.webp`) im Sidebar-Header, Platzhalter-Box entfernt. Auf Anfrage vereinfacht: Text-Label „Q7", BETA-Badge und „Tamza Assistent"-Block entfernt — nur noch Logo sichtbar. Achtung: „Tamza Assistent" war laut Doku das White-Label-Beispiel für Lizenznehmer-Identität (Black-Box-Prinzip) — falls das wieder gebraucht wird, siehe Q7-A-098-Hinweis unten |
| Typografie-Skala (Inter, 14px Basis, Zeilenhöhe 1.5) | 02.08. | `globals.css` global umgesetzt (Inter-Import muss vor `@import "tailwindcss"` stehen, sonst Build-Fehler). Navigation/Buttons auf 14px Medium vereinheitlicht in `Sidebar.tsx`, `AufgabenListe.tsx`, `VerlegenButton.tsx`. Seiten-Haupttitel bewusst bei 20px belassen (Admin-Entscheidung). Restliche Seiten noch nicht abgedeckt, siehe Q7-A-106 |

---

## K. Sitzung 02.08.2026 — Eingang/Aufgaben-Feature, Logo, Typografie, Coaching-Idee

| ID | Priorität | Thema | Beschreibung | Status |
|---|---|---|---|---|
| Q7-A-098 | mittel | Unternehmens-Chat für Mitarbeiter untereinander | Formalisierung des Admin-Nachtrags vom 01.08.2026 (siehe unten im Dokument): Mitarbeiter (z.B. Azubi) sollen künftig untereinander kommunizieren können, nicht nur mit dem Assistenten. Nicht Teil der Standard-Ausgabe (dort nur eine Person), erst relevant bei Multi-User-Ausbau | Offen — gekoppelt an Q7-A-024 (Multi-User-Umsetzung, bewusst zurückgestellt) |
| Q7-A-099 | hoch | Zwei parallele Datenbanken (agenten.db vs. q7_agenten.db) | `lib/db/agentenDb.ts` (better-sqlite3, Raw-SQL, Kapselungsregel Q7-A-001) zeigt auf `agenten.db` mit anderem `aufgaben`-Schema und anderen Status-Werten als das aktive Prisma-Modell (`q7_agenten.db`, genutzt von Kanal-Intake/MEINE AUFGABEN). `agentenDb.ts` scheint faktisch ungenutztes Altsystem zu sein — Kapselungsregel "kein Code außerhalb dieser Datei darf auf DB zugreifen" bereits gebrochen | Offen (02.08.2026) — Entscheidung nötig: `agentenDb.ts`/`agenten.db` löschen, oder Zweck klären und dokumentieren |
| Q7-A-100 | mittel | `GEPRUEFT_HERMES`-Status verstößt gegen Namenskonvention | `status`-Feld in `aufgaben`-Tabelle enthält Wert `GEPRUEFT_HERMES` — widerspricht der Konvention "ausschließlich A01, kein Eigenname 'Hermes' mehr" (31.07.2026). Umbenennung zu `GEPRUEFT_A01` erfordert Migration, da Wert ggf. bereits in Gebrauch | Offen (01.08.2026) |
| Q7-A-101 | niedrig | "GF"-Kommentare in `lib/types.ts` nicht bereinigt | Mehrere Code-Kommentare nutzen noch "GF-Entscheidung" statt "Admin-Entscheidung" (u.a. bei `AGENTS`, `PROTOKOLL_LIST`); zusätzlich Kommentar "Hermes bleibt informelle Zusatzbezeichnung für A01" widerspricht Namenskonvention vom 31.07.2026 | Offen (01.08.2026) — noch nicht korrigiert, bewusst zurückgestellt während Eingang/Aufgaben-Feature-Arbeit |
| Q7-A-102 | hoch | Freigabe-Aufgaben-Quelle (MEINE AUFGABEN) nicht anbindbar | Dritte Aufgaben-Quelle laut `Q7_Freigabe_Rechte_Aufgaben_v1_1.md` Abschnitt 3 (Agent-Ergebnis fertig → Freigabe-Task) hängt an der `ergebnisse`-Tabelle, die noch nicht real angelegt ist | Offen — blockiert durch Q7-A-001 |
| Q7-A-103 | mittel | Automatische KI-Vorschlagserkennung Eingang→Aufgaben | Aktuell (V1) nur manueller "→ Zu Aufgaben"-Button auf der Eingang-Seite. Ursprüngliches Konzept: A01 erkennt automatisch Handlungsbedarf (z.B. "ruf mich zurück" in WhatsApp/E-Mail) und schlägt Verlegung proaktiv vor, mit Bestätigungspflicht. **Admin-Entscheidung 02.08.2026:** Klassifizierung bleibt mit Rückfrage, kein Standing-Order-Lernen (Abweichung von Q7-A-061-Logik für andere Tätigkeitstypen, hier bewusste Ausnahme). **Revidiert 05.08.2026 (siehe Q7-A-117–120):** "Dauerhaft" gilt nur für die aktuelle Testphase — nach Testphase mit Peters eigener Firma darf A01 auch hier lernen, über die Serien-Logik aus Abschnitt M | Offen (05.08.2026) — Konzept erweitert, technische Umsetzung weiterhin nicht begonnen |
| Q7-A-104 | mittel | Chat-Antwortzeit bei Gesprächsbeginn ungewöhnlich lang | Nur beim Begrüßen/Start eines neuen Chats spürbar langsamer, nicht bei Folgenachrichten. Vermutung (nicht verifiziert): möglicher zweiter, sequenzieller `rufeKIAn()`-Aufruf für automatische Chat-Titel-Generierung (`titelStatus: "pending"|"final"` in `ChatThread`-Typ deutet darauf hin) | Offen (02.08.2026) — `app/api/chat/route.ts` zur Bestätigung noch nicht eingesehen |
| Q7-A-105 | niedrig | Favicon/Kleinformat-Test Q7-Logo | Logo im Sidebar-Header eingebaut und final. Favicon-Verwendung sowie Kleinformat-Lesbarkeit (laut Tageszusammenfassung 01.08.2026 als offener Punkt vermerkt) noch nicht getestet | Offen |
| Q7-A-106 | niedrig | Typografie-Skala auf restliche Seiten ausrollen | Inter/14px/Medium-Regel bisher nur in `Sidebar.tsx`, `AufgabenListe.tsx`, `VerlegenButton.tsx`, `input/page.tsx`, `aufgaben/page.tsx` umgesetzt. Kalender, Studio, Spaces, Output, Protokoll, Ablage, Einstellungen noch nicht abgeglichen (teils noch nicht gebaut, siehe Q7-A-107) | Offen |
| Q7-A-107 | hoch | Termin-Datenmodell fehlt | Voraussetzung für: (a) vier Kalender-Ansichtsmodi (Q7-A-045), (b) Routing-Fix `/kalender` (Q7-A-051), (c) geplante Coaching-/Erinnerungsfunktion (Q7-A-108). Kein Prisma-Modell, keine Seite, keine Tabelle vorhanden | Offen (02.08.2026) — Kernabhängigkeit für mehrere andere Punkte |
| Q7-A-108 | mittel | A01-Coaching-Verhalten bei Terminerinnerungen (Zusatzfunktion, Idee) | Admin-Idee: A01 erinnert 1h vor wichtigen Terminen proaktiv, mit coachendem Ton (Beispiel: "Meeting XY in 1h, bereite dich vor, das hat höchste Priorität"). Nutzt Kenntnis aus Eingang/Aufgaben/Kalender zur Personalisierung. **Bestätigt:** nur innerhalb der Q7-App (kein WhatsApp), Kombination aus In-App-Banner + E-Mail — **Klärungsbedarf:** Formulierung "nur in der Q7 App" und "Kombi aus A+B" (wobei B=E-Mail, außerhalb der App) sind im Wortlaut nicht ganz deckungsgleich, bei Umsetzung nochmal bestätigen. Braucht: Termin-Datenmodell (Q7-A-107), Definition "wichtig" (manuelles Flag vs. Schlüsselwörter), dokumentierten Kommunikationsstil (SSOT), Scheduled-Task analog `run-poll-gmail.bat` | Offen (02.08.2026) — Konzept, keine technische Umsetzung begonnen. Zusatzfunktion, siehe Q7-A-110 (Pricing) |
| Q7-A-109 | niedrig | Verkehrsdaten-Integration für Coaching-Feature (optional) | Erweitertes Coaching-Beispiel (Terminempfehlung wegen Stau/Baustelle) würde externe Verkehrsdaten-API (z.B. Google Maps/HERE) erfordern — komplett neue Integration. Steht in Spannung zum "kein Monster züchten"-Prinzip (25.07./01.08.) — Abstraktion soll aus echtem, wiederkehrendem Bedarf entstehen, nicht aus Wunschliste | Offen (02.08.2026) — Bedarfsklärung aussteht, ob wiederkehrend oder Einzelfall-Idee |
| Q7-A-110 | mittel | Coach als Feature-Flag: Add-on oder Stufengebunden | Admin: Coaching-Funktion (Q7-A-108) soll abschaltbar sein bzw. nur in Pro-Version — deckt sich nicht 1:1 mit bestehendem Stufenmodell (Small/Medium/Enterprise, unterschieden nach Space-Limits). Klären: neue unabhängige Feature-Flag-Dimension (zubuchbares Add-on für jede Stufe) vs. an bestimmte Stufe gekoppelt (z.B. nur ab Medium) | Offen (02.08.2026) — Admin-Formulierung ließ beide Varianten offen, keine Endentscheidung |
| Q7-A-111 | hoch | Fachagenten-Weiterleitung (Schritt 6 Kanal-Intake) | A01 setzt aktuell nur `ziel_agent` in der `aufgaben`-Tabelle — es gibt noch keinen Mechanismus, der den jeweiligen Fachagenten (z.B. A05_QM) tatsächlich mit `rufeKIAn()` aufruft und eine Antwort erzeugt. Hängt zusätzlich an `ergebnisse`-Tabelle (siehe Q7-A-102) | Offen (31.07./02.08.2026) — Kernstück für vollständigen Kanal-Intake-Ablauf, bisher nur Schritte 1–5 real funktionsfähig |

---

## L. Sitzung 05.08.2026 — ERP-Zugriffsentscheidung, Posteingang-Kanal-Tabs

Kontext: Sidebar zeigte kurzzeitig einen neuen Punkt "ERP" (Neu-Badge). Klärung ergab: keine eigene ERP-Rubrik nötig, Zugriff läuft über bestehende Bereiche. Parallel: Posteingang um Kanal-Tabs (Team/Mail/WhatsApp/Telegram/Facebook) + Antwort-Panel erweitert.

| ID | Priorität | Thema | Beschreibung | Status |
|---|---|---|---|---|
| Q7-A-112 | hoch | ERP-Zugriff — keine eigene Sidebar-Rubrik | Entschieden 05.08.2026: ERP-Zugriff läuft über drei bestehende Wege statt eigenem Cockpit — (1) Studio: schreibende Werkzeuge ("Angebot generieren", "Lieferschein schreiben"); (2) Dashboard/Überblick: lesende KPI-Kacheln (Konzeptdokument Abschnitt 3.1); (3) KI-Assistent: Ad-hoc-Abfragen im Chat. Details siehe `Q7_ERP_und_Integrationsplattform.md` Abschnitt "Q7-seitige Zugriffswege" (v1.1) | Entschieden — technischer Rückbau (Sidebar.tsx-Eintrag "ERP" entfernen) offen |
| Q7-A-113 | mittel | Studio-Werkzeug "Lieferschein schreiben" fehlt | Analog zu "Angebot generieren" (A03 Vertrieb) noch zu spezifizieren: Agent-Zuordnung, Eingabefelder, Datenfluss über Integrationsplattform (Schreibrichtung, Standard-Schnittstelle Verkauf/Lieferschein) | Offen (05.08.2026) |
| Q7-A-114 | mittel | KI-Assistent Ad-hoc-ERP-Abfrage — Datenfluss unspezifiziert | Dritter Zugriffsweg (Chat fragt ERP-Daten ab, z.B. "Wie viele offene Angebote hat Kunde X?") technisch noch nicht beschrieben. Vermutlich rufeKIAn()-Tool-Call an Integrationsplattform, analog Studio-Schreibzugriff aber lesend | Offen (05.08.2026) |
| Q7-A-115 | mittel | Posteingang Kanal-Tabs + Antwort-Panel | UI erweitert: Tabs (Alle/Team/Mail/WhatsApp/Telegram/Facebook), Split-Layout Liste+Detail, Antwort wird in `AufgabeChat` gespeichert (kein echter Kanal-Versand, da Connectoren für WhatsApp/Telegram/Facebook/Mail-Ausgang noch nicht existieren — UI zeigt entsprechenden Hinweis) | **Teilerledigt 05.08.2026** — UI gebaut (`app/input/page.tsx`, `PosteingangClient.tsx`, `app/api/aufgabe-chat/route.ts`), noch nicht im Projekt getestet. `team`-Kanal hat noch keine reale Datenquelle |
| Q7-A-116 | niedrig | Sidebar-Struktur-Doku hinter Ist-Zustand zurück | v3 (01.08.) enthielt weder "Überblick" noch "Posteingang"-Umbenennung als Ist-Stand. Neue Version v4 (05.08.) nachgezogen | **Erledigt 05.08.2026** — `Q7_Sidebar_Struktur_2026-08-05_v4.md` |

## M. Sitzung 05.08.2026 (Teil 2) — A01-Lernmechanismus für Eingang→Aufgaben-Verlegung

Kontext: Vertiefung von Q7-A-103. Ausgangsfrage: ab wann darf A01 bei der Eingang→Aufgaben-Klassifizierung (z.B. Rückruf-Wünsche, die nur ein Mensch ausführen kann) ohne Rückfrage entscheiden? Admin lehnt starren Prozentwert (z.B. 98% Trefferquote) ab — Serien-Logik statt Gesamt-Statistik, da eine Trefferquote über die gesamte Historie eine schlechte aktuelle Performance verdecken kann.

**Entschiedener Mechanismus:**

1. **Serien-Zählung pro Anliegen-Typ getrennt** (Rückruf, Reklamation, Terminanfrage, ... jeweils eigene Serie, nicht global) — jeder Admin-Widerspruch setzt die Serie der betroffenen Kategorie auf 0 zurück.
2. **Schwellenwert konfigurierbar** pro Lizenznehmer/Kategorie (kein Festwert im Code, Standardvorschlag 10 als Ausgangspunkt).
3. **Zwei-Stufen-Bestätigung (Cooling-off) beim Umschalten auf Autonomie:**
   - Tag 1 (Schwelle erreicht): A01 fragt — "Ich habe [N]x richtig entschieden. Soll ich bei '[Kategorie]' künftig ohne Rückfrage verlegen?"
   - Tag 2 (nächster Tag bzw. nächster neuer Task dieser Kategorie): A01 fragt erneut — "Gestern hast du entschieden, dass ich ab sofort ohne deine Freigabe die Eingänge zuordnen kann. Falls du immer noch der Meinung bist, bestätige mir das. Wenn du weiter kontrollieren möchtest, klicke auf 'Später umstellen'."
   - Erst nach der zweiten Bestätigung wird die Kategorie autonom; bis dahin bleibt jede Verlegung rückfragepflichtig.
4. Q7-A-103s "dauerhaft kein Lernen" gilt damit nur für die aktuelle Testphase (Admin testet zunächst mit eigener Firma) — danach greift dieser Mechanismus.

| ID | Priorität | Thema | Beschreibung | Status |
|---|---|---|---|---|
| Q7-A-117 | hoch | Serien-Zähler pro Anliegen-Typ (Datenmodell) | Neues Feld/Tabelle benötigt, z.B. `lizenznehmer_lernstatus` (anliegen_typ, serie_aktuell, schwellenwert, autonomie_status: manuell\|warte_bestaetigung_tag2\|autonom) | Offen (05.08.2026) |
| Q7-A-118 | hoch | Zwei-Stufen-Bestätigungs-Dialog UI | A01-Frage-Flow mit Tag-1- und Tag-2-Formulierung (siehe Wortlaut oben), inkl. "Später umstellen"-Option, die den Status auf 'manuell' zurücksetzt statt zu löschen | Offen (05.08.2026) |
| Q7-A-119 | mittel | Schwellenwert-Konfiguration UI | Einstellbar pro Lizenznehmer und Kategorie — vermutlich Teil der bereits geplanten Delegation-Rubrik in Einstellungen (Q7-A-053) | Offen (05.08.2026) — Abhängigkeit zu Q7-A-053 |
| Q7-A-120 | niedrig | Reset-Verhalten bei Admin-Widerspruch definieren | Bestätigt: harter Reset auf 0 bei jedem Widerspruch (keine Toleranz-Pufferung/Decay) — einfachste, konservativste Regel, passt zu "Kein Monster züchten" | **Entschieden 05.08.2026**, technische Umsetzung siehe Q7-A-117 |
...
*(Formalisiert 02.08.2026 als Q7-A-098, siehe Abschnitt K.)*





- **v3.5 (05.08.2026):** Neuer Abschnitt L (Q7-A-112–116) — ERP-Zugriffsentscheidung (keine eigene Sidebar-Rubrik, drei bestehende Zugriffswege: Studio/Dashboard/KI-Assistent), Posteingang-Kanal-Tabs + Antwort-Panel als Teilerledigt vermerkt, Sidebar-Struktur-Doku auf v4 nachgezogen.

- **v3.6 (05.08.2026, Nachtrag):** Neuer Abschnitt M (Q7-A-117–120) — A01-Lernmechanismus für Eingang→Aufgaben-Verlegung (Serien-Logik pro Anliegen-Typ statt Prozent-Trefferquote, Zwei-Stufen-Cooling-off-Bestätigung). Q7-A-103 revidiert: "dauerhaft kein Lernen" gilt nur für aktuelle Testphase.
...
- **v3.4 (02.08.2026):** Sitzung Eingang/Aufgaben-Feature, Logo, Typografie, Coaching-Idee eingearbeitet. Status-Updates bestätigt funktionsfähiger Punkte: Q7-A-085, 087, 088, 091, 095 (teilerledigt), 096, 097 auf Erledigt/Teilerledigt gesetzt (End-to-End-Test 01.08.2026 erfolgreich: Gmail→Poll→DB→Klassifikation→Eingang-UI). Q7-A-051 teilerledigt (/aufgaben behoben, /kalender weiterhin offen). Q7-A-077/078 geklärt — Admin bestätigt Fremd-SYSTEM-PROMPT inkl. 3-Stufen-Modell, Kurskorrektur gegenüber früherer Verwerfen-Empfehlung. Neuer Abschnitt K (Q7-A-098–111): Eingang/MEINE-AUFGABEN-Feature real gebaut (Details in "Bereits erledigt"), zwei parallele Datenbanken als neuer Konflikt identifiziert (Q7-A-099), Namenskonvention-Verstoß `GEPRUEFT_HERMES` (Q7-A-100), Logo + Typografie-Skala umgesetzt (offene Reste: Favicon-Test Q7-A-105, Rollout restliche Seiten Q7-A-106), Termin-Datenmodell als neue Kernabhängigkeit identifiziert (Q7-A-107), Coaching-Assistent-Konzept aufgenommen (Q7-A-108–110), Fachagenten-Weiterleitung als expliziter offener Punkt (Q7-A-111), Unternehmens-Chat-Nachtrag formalisiert (Q7-A-098).
